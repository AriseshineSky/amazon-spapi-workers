...
def send_spapi_item_offers_update_task(
    broker_url, condition="new", ttl=36, force=False
):
        ...
        while True:
            for marketplace in marketplaces:
                ...
                if not self.product_service.ensure_indice(
                    asin_indice_name, ASIN_NO_OFFER_INDEX_BODY
                ):
                    logger.warning(
                        "Skipping %s: index %s missing, could not create "
                        "(shard limit); add index manually.",
                        marketplace,
                        asin_indice_name,
                    )
                    continue
                try:
                    for (
                        s,
                        _,
                    ) in self.product_service.load_products_by_after_search(
                        asin_indice_name
                    ):
